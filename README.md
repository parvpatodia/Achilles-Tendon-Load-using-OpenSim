...
- **The worst athlete, not just the average.** Most held-out athletes are modelled very well (median loaded R² 0.96), but the single worst scores 0.47. We looked at why: that athlete loads the tendon less than the cohort, so a model trained on everyone else over-predicts their push-off force by about 1.2 BW. That is a systematic per-person bias, exactly what the per-athlete calibration step (§9) removes, not random scatter. A wearable is judged on its weakest case, so we report it plainly.
- **It generalizes to a load regime it never trained on.** We held out an entire running speed and trained on the others: the surrogate still scores loaded R² ≥ 0.94 at every held-out speed, including extrapolating down to the slowest (toward walking-like loads) with near-zero peak bias. This isolates load-regime shift (subjects appear at all speeds); new-subject generalization is the separate k-fold result above. It is encouraging, not proof, for transfer to your walking cohort, which needs walking GRF from the real insole to test directly (the public walking set has none).
...
### 6j. Measured pressure instead of the proxy: pipeline built, data audited

Everything above feeds the surrogate a *modelled* four-zone proxy (derived from total push, §6c). Mirai's product is a real multi-zone pressure insole, so the question that matters is: does measured plantar pressure replace the proxy? This stage builds the full path to answer it and, on a public measured-insole dataset, audits whether that data can honestly do so.

- **The target, computed ourselves.** The public running data ships pre-computed joint moments; a measured-insole walking dataset (Grouvel et al. 2023, Moticon 16-zone insoles + marker mocap + force plates, CC-BY) ships raw motion only. So we run the musculoskeletal pipeline end to end: `c3d -> .trc/.mot -> OpenSim scale -> IK -> inverse dynamics -> Achilles`. On subject P02 walking this reaches **1.9 cm marker RMS** (research-grade) and a clean **2.0 body-weight** push-off, matching the walking literature. This is the same physics the running cross-check uses (§6d), computed from scratch on a second dataset.
- **The input, made pluggable.** A trial can now carry real measured zones (`GaitTrial.measured_zones`); the feature builder swaps the modelled CoP bump for the measured pressure share while holding vGRF, ankle angle, target, and architecture fixed, so measured-vs-derived is a single controlled comparison (`build_features(..., use_measured=)`). `GrouvelDataSource` reads committed per-stance CSVs, so consuming it needs neither OpenSim nor the 2.6 GB archive.
- **The honest audit (why no measured numbers are reported yet).** Before training on the measured pressure, the harvester checks that the insole total force actually tracks the synchronised force plate over each stance. It does not: on P02 the insole-vs-plate correlation is **r ~= 0.1-0.3** (literature is r > 0.9), and the ~31 Hz insole resampled onto the 100 Hz mocap grid loses the per-stance timing (peak magnitudes agree, ~1.0 BW, but the shape does not; a lag scan cannot recover it). The 16 sensors are a 2D grid, not a heel-to-toe line, so no fixed anatomical grouping is assumed. The harvester therefore **refuses every P02 stance** rather than train the surrogate on pressure that does not match ground truth. That refusal is the result: the pipeline is validated and ready, and it correctly rejects bad input. Mirai's own insole, with its own reference, drops straight into this path once its pressure passes the same trust gate.
